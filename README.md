...
이 프로젝트는 백엔드(Spring Boot), 프론트엔드(Vue), 그리고 채점 서버(Judge0 Docker)로 구성. 프로젝트 시작 개발자는 아래 가이드 따라 개발 환경 설정 및 실행 가능.

**자세한 단계별 가이드는 `.Ref/실행 방법.txt` 파일 참조.**
...
0.  작업 관리자 > 서비스 > MYSQL80 > 오른쪽 클릭 (시작)
1.  **Judge0 채점 서버 실행:**
